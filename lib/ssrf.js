...
export function isPrivateIp(ip) {
  ...
  if (value.includes(":")) {
    ...
    if (n === 0x2001 && value.split(":")[1]?.toLowerCase() === "db8") return true;
    return false;
  }
...
export function pickAddress(records) {
  const safe = (records || []).filter((record) => record?.address && !isPrivateIp(record.address));
  if (!safe.length) return null;
  return safe.find((record) => !String(record.address).includes(":")) || safe[0];
}

export function pinnedLookup(address) {
  const family = String(address).includes(":") ? 6 : 4;
...
export function dispatcherFor(address) {
  return new Agent({
    connect: {
      timeout: 10_000,
...
export async function resolvePublic(input) {
  ...
  const chosen = pickAddress(records);
  if (!chosen) throw fail(400, "That URL is not allowed.");
