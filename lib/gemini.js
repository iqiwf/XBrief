...
function providerDetail(status, message) {
  return String(message || `Gemini returned ${status}.`).replace(/\s+/g, " ").slice(0, 180);
}

function overloaded(status, message) {
  return status === 503 || /high demand|overloaded|unavailable/i.test(message || "");
}

async function generate(apiKey, model, mode, article, issues, attempt = 0) {
  ...
  if (!response.ok) {
    ...
    const detail = providerDetail(response.status, message);
    console.error(`[gemini] ${response.status} ${detail}`);
    if (attempt < 1 && overloaded(response.status, message)) {
      await new Promise((resolve) => setTimeout(resolve, 800));
      return generate(apiKey, model, mode, article, issues, attempt + 1);
    }
    if (overloaded(response.status, message)) {
      throw fail(503, "Gemini is busy right now. Try again in a moment.");
    }
...
async function generateLegacy(apiKey, model, mode, article, issues) {
  ...
  if (!response.ok) {
    const message = body?.error?.message || `Gemini returned ${response.status}.`;
    console.error(`[gemini] legacy ${response.status} ${providerDetail(response.status, message)}`);
    if (overloaded(response.status, message)) {
      throw fail(503, "Gemini is busy right now. Try again in a moment.");
    }
    throw fail(502, "Gemini could not write the post. Try again.");
...
export async function writePosts({ apiKey, model, mode, article }) {
  ...
  let repairs = 0;
  const repair = (issues) => {
    if (repairs >= 1) return null;
    repairs += 1;
    return generate(apiKey, model, mode, article, issues);
  };
