...
async function generate(apiKey, model, mode, article, issues) {
  ...
  if (!response.ok) {
    ...
    const detail = String(message || `Gemini returned ${response.status}.`).replace(/\s+/g, " ").slice(0, 180);
    console.error(`[gemini] ${response.status} ${detail}`);
    throw fail(502, `Gemini could not write the post. ${detail}`);
  }
...
async function generateLegacy(apiKey, model, mode, article, issues) {
  ...
  if (!response.ok) {
    const message = body?.error?.message || `Gemini returned ${response.status}.`;
    const detail = String(message).replace(/\s+/g, " ").slice(0, 180);
    console.error(`[gemini] legacy ${response.status} ${detail}`);
    throw fail(502, `Gemini could not write the post. ${detail}`);
  }
  const parts = body?.candidates?.[0]?.content?.parts || [];
