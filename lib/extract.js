...
async function fetchHtml(start) {
  ...
  for (let hop = 0; hop <= MAX_REDIRECTS; hop += 1) {
    ...
    if (response.status >= 300 && response.status < 400) {
      ...
      current = await assertPublicHttpUrl(new URL(next, current));
      continue;
    }
    if (response.status === 401 || response.status === 403) {
      throw fail(422, "The site refused the request. It may require a login.");
    }
    if (response.status === 404) throw fail(422, "That page was not found.");
    if (!response.ok) throw fail(502, `The site returned ${response.status}.`);
    ...
    if (length > MAX_BYTES) throw fail(422, "That page is too large to read.");
    const buffer = Buffer.from(await response.arrayBuffer());
    if (buffer.length > MAX_BYTES) throw fail(422, "That page is too large to read.");
...
function clip(text) {
  if (text.length <= MAX_ARTICLE_CHARS) return { text, truncated: false };
  const slice = text.slice(0, MAX_ARTICLE_CHARS);
  const cut = slice.lastIndexOf("\n\n");
  return { text: (cut > 4000 ? slice.slice(0, cut) : slice).trim(), truncated: true };
}
...
export function parseArticle(html, finalUrl) {
  ...
  return {
    ...
    words: clipped.text.split(/\s+/).filter(Boolean).length,
  };
}
