...
function jsonLdText(document) {
  ...
  for (const node of document.querySelectorAll("script[type='application/ld+json']")) {
    ...
    for (const item of items) {
      ...
      if (typeof body === "string" && body.trim().length > blocks.join("").length) blocks.splice(0, blocks.length, body.trim());
    }
  }
  return blocks[0] || "";
}
...
function bodyFrom(root) {
  ...
  const text = blocks.length >= 2 ? blocks.map((node) => node.textContent).join("\n\n") : root.textContent || "";
  return text;
}
